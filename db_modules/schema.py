# -*- coding: utf-8 -*-
"""
Database schema definitions for POS Offline.
Contains all 33 CREATE TABLE statements, default settings, and helper functions.
"""

# All CREATE TABLE IF NOT EXISTS statements for the tenant/default database.
# These cover all 33 tables (30 tenant tables + 3 master tables defined in master.py).
TABLES_SQL = '''
...
# No CREATE INDEX statements exist in the current schema, but this is
# provided as a placeholder for future use.
INDEXES_SQL = ''

# Default settings inserted into the settings table.
# The first 8 are the "all_settings" set (used by init_default_db).
# The first 3 + store_name + currency + tax_enabled + low_stock_threshold
# are the "basic" set (used by ensure_db_tables).
...
# Extended settings used by migrate_database for older databases
DEFAULT_SETTINGS_MIGRATE = [
    ('store_name', '\u0645\u062a\u062c\u0631 \u0627\u0644\u0639\u0637\u0648\u0631 \u0648\u0627\u0644\u0628\u062e\u0648\u0631'),
    ('store_phone', ''),
    ('store_address', ''),
    ('tax_enabled', 'false'),
...
def create_indexes(cursor):
    """Execute all CREATE INDEX statements (currently none defined)."""
    if INDEXES_SQL.strip():
        cursor.executescript(INDEXES_SQL)


def insert_default_settings(cursor, all_settings=True):
    """Insert default settings into the settings table.

    Args:
        cursor: sqlite3 cursor
        all_settings: If True, insert the full set of default settings
                      (loyalty, store, currency, tax, low_stock).
                      If False, insert only the basic loyalty + store settings.
    """
    if all_settings:
        for key, value in DEFAULT_SETTINGS_ALL:
            cursor.execute(
                "INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)",
                (key, value)
            )
    else:
        # Basic settings used by ensure_db_tables
        basic = [
            ('loyalty_points_per_invoice', '10'),
            ('loyalty_point_value', '0.1'),
            ('loyalty_enabled', 'true'),
...
def insert_default_branch(cursor):
    """Insert the default branch if it doesn't exist."""
    cursor.execute(
        "INSERT OR IGNORE INTO branches (id, name, location, is_active) VALUES (1, '\u0627\u0644\u0641\u0631\u0639 \u0627\u0644\u0631\u0626\u064a\u0633\u064a', '', 1)"
    )
